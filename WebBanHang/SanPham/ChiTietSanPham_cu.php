...
function ajaxAddToCart() {
    ...
    // Lưu ý: $('input[name="product_id"]') lấy giá trị từ thẻ input có name là product_id
    ...
    // 3. Kiểm tra dữ liệu
    if (!variant_id) {
        alert("Vui lòng chọn biến thể (Màu sắc/Cấu hình)!");
        $('#id_bien_the').focus();
        return;
    }
    if (quantity < 1) {
    ...
    console.log("Đang gửi yêu cầu...", {product_id, variant_id, quantity});

    // 4. Gửi AJAX
    $.ajax({
        url: '../DonHang/api_cart.php', // Đảm bảo file này đã được tạo ở bước trước
        type: 'POST',
        dataType: 'json',
        data: {
            action: 'add',
        ...
        success: function(response) {
            ...
            if (response.status === 'success') {
                // Hiện thông báo thành công (Popup xanh)
                // Nếu bạn có div id="successCartMsg" thì hiện nó lên
                if(document.getElementById('successCartMsg')) {
                    document.getElementById('successCartMsg').style.display = 'block';
                } else {
                    alert(response.message); // Fallback nếu không có popup
                }
            } else {
